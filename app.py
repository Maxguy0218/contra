...
def create_vector_store(texts):
    try:
        # Use local embeddings to avoid HF connection issues
...
def tools_page():
    ...
    with col2:
        ...
        st.write("- Renewal Tracker")
        st.write("- Obligation Manager")
        st.write("- Approval Workflows")
        st.write("- Notification System")

    st.markdown("---")
    st.subheader("Recent Activities")
    activity_data = pd.DataFrame({
        "Activity": ["Contract Review", "Risk Assessment", "Renewal Check", "Compliance Audit"],
...
def analytics_page():
    ...
    with tab1:
        ...
        with col1:
            data = pd.DataFrame({
                'Quarter': ['Q1', 'Q2', 'Q3', 'Q4'],
                'Revenue ($M)': [45, 67, 55, 80]
            })
            fig = px.bar(data, x='Quarter', y='Revenue ($M)', color='Quarter',
                         color_discrete_sequence=[FEDEX_PURPLE, FEDEX_ORANGE, "#333333", "#666666"])
            st.plotly_chart(fig, use_container_width=True)
        
        with col2:
            data = pd.DataFrame({
                'Category': ['Services', 'Software', 'Hardware', 'Maintenance'],
                'Value': [35, 25, 20, 20]
            })
    ...
    with tab2:
        ...
        with col2:
            data = pd.DataFrame({
                ...
                'Contracts Processed': [120, 150, 180, 210]
            })
            fig = px.line(data, x='Month', y='Contracts Processed',
                          color_discrete_sequence=[FEDEX_PURPLE])
            st.plotly_chart(fig, use_container_width=True)
