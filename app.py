...
def tools_page():
    ...
    st.write("This is the dedicated tools page")
    
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Document Tools")
        st.write("- Contract Analyzer")
        st.write("- Clause Generator")
        st.write("- Risk Calculator")
    
    with col2:
        st.subheader("Automation Tools")
        st.write("- Renewal Manager")
        st.write("- Compliance Auditor")
        st.write("- Obligation Tracker")

def analytics_page():
    st.markdown("## Advanced Analytics")
    
    tab1, tab2 = st.tabs(["Financial Metrics", "Operational Metrics"])
    ...
    with tab1:
        ...
        fig = px.bar(data, x='Quarter', y='Revenue', color='Quarter',
    ...
    with tab2:
        ...
        data = pd.DataFrame({
            'Metric': ['On-time Delivery', 'Customer Satisfaction', 'Contract Compliance'],
            'Score': [92, 88, 95]
        })
        fig = px.pie(data, values='Score', names='Metric',
...
def main():
    ...
    # Custom CSS with fixed header and navigation
    st.markdown(f"""
        <style>
    ...
    # Navigation handling with proper session state
    if 'nav' not in st.session_state:
        st.session_state.nav = 'home'
        
    # Navigation buttons using proper Streamlit components
    with st.container():
        col1, col2 = st.columns([1, 15])
        with col1:
            st.markdown(f"""
                <div class="nav-container">
                    <button class="nav-button {'active' if st.session_state.nav == 'home' else ''}" 
                        onclick="window.parent.postMessage('home', '*')">
                        <i class="fas fa-home nav-icon"></i>
                        <span class="nav-label">Home</span>
                    </button>
                    <div style="height: 25px"></div>
                    <button class="nav-button {'active' if st.session_state.nav == 'tools' else ''}" 
                        onclick="window.parent.postMessage('tools', '*')">
                        <i class="fas fa-tools nav-icon"></i>
                        <span class="nav-label">Tools</span>
                    </button>
                    <div style="height: 25px"></div>
                    <button class="nav-button {'active' if st.session_state.nav == 'analytics' else ''}" 
