...
def render_history_page():
    st.title("History")
    st.write("View your contract history and previous analyses here")
    st.image("https://via.placeholder.com/800x400.png?text=History+Dashboard", use_column_width=True)

def render_playbook_page():
    st.title("Playbook")
    st.write("Access contract templates and best practices")
    st.image("https://via.placeholder.com/800x400.png?text=Playbook+Content", use_column_width=True)

def render_settings_page():
    st.title("Settings")
    st.write("Configure your application preferences")
    st.image("https://via.placeholder.com/800x400.png?text=Settings+Panel", use_column_width=True)

def render_contact_page():
    st.title("Contact Us")
    st.write("Get in touch with our support team")
    st.image("https://via.placeholder.com/800x400.png?text=Contact+Information", use_column_width=True)
...
def main():
    ...
    with st.container():
        ...
        with col2:
            ...
            pages = [
                ...
                ("Playbook", render_playbook_page),
                ("Settings", render_settings_page),
                ("Contact Us", render_contact_page)
            ]
            
            for i, (page_name, _) in enumerate(pages):
                with cols[i]:
                    if st.button(
                        page_name,
                        key=f"nav_{page_name}",
                        on_click=lambda p=page_name: st.session_state.update({"current_page": p})
                    ):
                        pass
    ...
    for page_name, render_func in pages:
