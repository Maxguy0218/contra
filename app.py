...
def tools_page():
    st.markdown("## Contract Tools Dashboard")
    st.write("Contract management tools would appear here")
    
    col1, col2 = st.columns(2)
    
    with col1:
        st.subheader("Contract Generator")
        st.text_area("Enter contract details", height=200)
        st.button("Generate Contract")
        
    with col2:
        st.subheader("Clause Library")
        st.selectbox("Select clause type", ["Termination", "Payment", "Confidentiality"])
        st.text_area("Clause text", height=200)
...
def analytics_page():
    ...
    with col2:
        ...
        st.metric("Total Active Contracts", "247", "+12 from last quarter")
        st.metric("Total Contract Value", "$48.7M", "3.2% increase")
        st.metric("Expiring Soon", "18", "5 need attention")
    ...
    st.markdown(f"""
        <style>
            ...
            .sidebar {{
                ...
                top: 0;
                bottom: 0;
                width: {'200px' if st.session_state.menu_expanded else NAV_WIDTH};
                background: {FEDEX_PURPLE};
                z-index: 1000;
                transition: 0.3s;
            }}
            ...
            /* Main Content */
            .main-content {{
                margin-left: {'200px' if st.session_state.menu_expanded else NAV_WIDTH};
    ...
    st.markdown(f"""
        <div class="sidebar">
            <button class="menu-button" onclick="window.parent.postMessage('Home', '*')">
                ...
                {'<span>Home</span>' if st.session_state.menu_expanded else ''}
            </button>
            <button class="menu-button" onclick="window.parent.postMessage('Tools', '*')">
                <i class="fas fa-tools menu-icon"></i>
                {'<span>Tools</span>' if st.session_state.menu_expanded else ''}
            </button>
            <button class="menu-button" onclick="window.parent.postMessage('Analytics', '*')">
                <i class="fas fa-chart-bar menu-icon"></i>
                {'<span>Analytics</span>' if st.session_state.menu_expanded else ''}
            </button>
            <button class="menu-button toggle-button" onclick="window.parent.postMessage('ToggleMenu', '*')">
                <i class="fas fa-chevron-{'left' if st.session_state.menu_expanded else 'right'} menu-icon"></i>
            </button>
        </div>
        <script>
